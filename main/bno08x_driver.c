...
#include "esp_heap_caps.h" // Ensure DMA memory allocation functions are available

// Define a default stack size if not provided by Kconfig
#ifndef CONFIG_ESP32_BNO08X_DATA_PROC_TASK_SZ
...
void BNO08x_init(BNO08x *device, BNO08x_config_t *imu_config)
{
    ...
    uint8_t* dma_init_tx_buffer = heap_caps_malloc(1, MALLOC_CAP_DMA);
    if (!dma_init_tx_buffer) {
        ESP_LOGE(TAG, "Failed to allocate DMA for init stabilization byte");
        ESP_ERROR_CHECK(ESP_ERR_NO_MEM); // Critical failure
    }
    dma_init_tx_buffer[0] = 0; // Value to send (e.g., 0x00 or 0xFF)

    device->spi_transaction.length = 8; // 8 bits for 1 byte
    device->spi_transaction.rxlength = 0;
    device->spi_transaction.tx_buffer = dma_init_tx_buffer;
    ...
    heap_caps_free(dma_init_tx_buffer); // Free after use
}
...
bool BNO08x_initialize(BNO08x *device)
{
    ...
    if (device->cb_list == NULL) {
        ESP_LOGE(TAG, "initialize (CS_Pin=%d): failed to allocate memory for cb_list.", device->imu_config.io_cs);
        // TODO: Add cleanup logic here for tasks/queues if partially created
        return false; // Return failure early
    }
    ...
    // Consider adding error checking for task creation results
    BaseType_t spi_task_created = xTaskCreate(&BNO08x_spi_task, "bno08x_spi_task", 4096, (void *)device, 8, &(device->spi_task_hdl));
    BaseType_t data_proc_task_created = xTaskCreate(&BNO08x_data_proc_task, "bno08x_data_proc_task", CONFIG_ESP32_BNO08X_DATA_PROC_TASK_SZ, (void *)device, 7, &(device->data_proc_task_hdl));

    if (spi_task_created != pdPASS || data_proc_task_created != pdPASS) {
        ESP_LOGE(TAG, "initialize (CS_Pin=%d): Failed to create tasks (SPI:%d, DataProc:%d)", device->imu_config.io_cs, spi_task_created, data_proc_task_created);
        // TODO: Clean up potentially created task/queue/event group/cb_list
         if (device->spi_task_hdl) vTaskDelete(device->spi_task_hdl);
         if (device->data_proc_task_hdl) vTaskDelete(device->data_proc_task_hdl);
         if (device->evt_grp_spi) vEventGroupDelete(device->evt_grp_spi);
         if (device->evt_grp_report_en) vEventGroupDelete(device->evt_grp_report_en);
         // Queues are harder to delete safely if tasks might be using them.
         if (device->cb_list) free(device->cb_list);
        return false;
    }

    // Maybe add a small delay here to ensure tasks are scheduled?
    vTaskDelay(pdMS_TO_TICKS(50));

    if (!BNO08x_hard_reset(device)) {
        ESP_LOGE(TAG, "initialize (CS_Pin=%d): hard_reset failed.", device->imu_config.io_cs);
        // TODO: Add cleanup logic here for tasks/queues/event groups/cb_list
        if (device->spi_task_hdl) vTaskDelete(device->spi_task_hdl);
        if (device->data_proc_task_hdl) vTaskDelete(device->data_proc_task_hdl);
        if (device->evt_grp_spi) vEventGroupDelete(device->evt_grp_spi);
        if (device->evt_grp_report_en) vEventGroupDelete(device->evt_grp_report_en);
    ...
    ESP_LOGI(TAG, "initialize: hard_reset completed for CS_Pin=%d. Calling get_reset_reason.", device->imu_config.io_cs);
    uint8_t reason_code = BNO08x_get_reset_reason(device);
    ESP_LOGI(TAG, "initialize: get_reset_reason returned %u for CS_Pin=%d", reason_code, device->imu_config.io_cs);

    if (reason_code != 0) // Reason codes are 1-5 for valid resets
    {
        ESP_LOGI(TAG, "Successfully initialized CS_Pin=%d....", device->imu_config.io_cs);
        return true;
    } else {
        ESP_LOGE(TAG, "initialize (CS_Pin=%d): get_reset_reason returned 0, initialization failed.", device->imu_config.io_cs);
        // TODO: Add cleanup logic here for tasks/queues/event groups/cb_list
        if (device->spi_task_hdl) vTaskDelete(device->spi_task_hdl);
        if (device->data_proc_task_hdl) vTaskDelete(device->data_proc_task_hdl);
        if (device->evt_grp_spi) vEventGroupDelete(device->evt_grp_spi);
    ...
    // Removed redundant checks from previous version
}
...
bool BNO08x_wait_for_data(BNO08x *device) {
    ...
    ESP_LOGD(TAG, "wait_for_data (CS_Pin=%d): Waiting for RX_DONE (HINT processed)...", device->imu_config.io_cs);
    // Wait for the SPI transaction triggered by HINT to complete and data to be queued.
    // Clear RX_DONE_BIT here as this call consumes the HINT event.
    events = xEventGroupWaitBits(device->evt_grp_spi,
                                 EVT_GRP_SPI_RX_DONE_BIT,
                                 pdTRUE,  // Clear EVT_GRP_SPI_RX_DONE_BIT on exit from this wait
                                 pdTRUE,  // Wait for ALL bits in this mask (just RX_DONE_BIT)
                                 HOST_INT_TIMEOUT_MS / portTICK_PERIOD_MS);

    if (events & EVT_GRP_SPI_RX_DONE_BIT) {
        ESP_LOGD(TAG, "wait_for_data (CS_Pin=%d): RX_DONE_BIT was set. Checking for packet processing status (VALID/INVALID)...", device->imu_config.io_cs);

        // Check the current status of VALID/INVALID bits *without waiting* and *without clearing*.
        // Data_proc_task should have already run for the packet associated with the RX_DONE we just consumed.
        events = xEventGroupGetBits(device->evt_grp_spi); // Read current bits

        if (events & EVT_GRP_SPI_RX_VALID_PACKET_BIT) {
            ESP_LOGI(TAG, "wait_for_data (CS_Pin=%d): VALID packet bit IS SET.", device->imu_config.io_cs);
             // Clear the bit now that we've seen it
            xEventGroupClearBits(device->evt_grp_spi, EVT_GRP_SPI_RX_VALID_PACKET_BIT);
            // Also clear INVALID just in case both got set somehow (shouldn't happen but safer)
            xEventGroupClearBits(device->evt_grp_spi, EVT_GRP_SPI_RX_INVALID_PACKET_BIT);
            success = true;
        } else if (events & EVT_GRP_SPI_RX_INVALID_PACKET_BIT) {
            ESP_LOGW(TAG, "wait_for_data (CS_Pin=%d): INVALID packet bit IS SET.", device->imu_config.io_cs);
            // Clear the bit now that we've seen it
            xEventGroupClearBits(device->evt_grp_spi, EVT_GRP_SPI_RX_INVALID_PACKET_BIT);
            // Also clear VALID just in case
            xEventGroupClearBits(device->evt_grp_spi, EVT_GRP_SPI_RX_VALID_PACKET_BIT);
            success = false; // Return FALSE for INVALID packets
        } else {
            // This case means neither VALID nor INVALID was set when we checked.
            // This could happen if data_proc_task hasn't finished yet.
            // Add a small delay and check again. If still not set, then timeout.
            ESP_LOGW(TAG, "wait_for_data (CS_Pin=%d): Neither VALID nor INVALID bit set immediately after RX_DONE. Delaying briefly...", device->imu_config.io_cs);
            vTaskDelay(pdMS_TO_TICKS(20)); // Allow data_proc_task more time
            events = xEventGroupGetBits(device->evt_grp_spi); // Check again

            if (events & EVT_GRP_SPI_RX_VALID_PACKET_BIT) {
                 ESP_LOGI(TAG, "wait_for_data (CS_Pin=%d): VALID packet bit IS SET after delay.", device->imu_config.io_cs);
                 xEventGroupClearBits(device->evt_grp_spi, EVT_GRP_SPI_RX_VALID_PACKET_BIT | EVT_GRP_SPI_RX_INVALID_PACKET_BIT);
                 success = true;
            } else if (events & EVT_GRP_SPI_RX_INVALID_PACKET_BIT) {
                 ESP_LOGW(TAG, "wait_for_data (CS_Pin=%d): INVALID packet bit IS SET after delay.", device->imu_config.io_cs);
                 xEventGroupClearBits(device->evt_grp_spi, EVT_GRP_SPI_RX_VALID_PACKET_BIT | EVT_GRP_SPI_RX_INVALID_PACKET_BIT);
                 success = false;
            } else {
                ESP_LOGE(TAG, "wait_for_data (CS_Pin=%d): Still Neither VALID nor INVALID packet status bit was set after delay.", device->imu_config.io_cs);
                success = false;
                 // Clear both bits to be safe for next time
                 xEventGroupClearBits(device->evt_grp_spi, EVT_GRP_SPI_RX_VALID_PACKET_BIT | EVT_GRP_SPI_RX_INVALID_PACKET_BIT);
            }
        }
    } else {
        ESP_LOGE(TAG, "wait_for_data (CS_Pin=%d): Timed out waiting for RX_DONE_BIT (HINT never asserted or SPI issue).", device->imu_config.io_cs);
        success = false;
        // Also clear potentially stale VALID/INVALID bits in case of timeout
         xEventGroupClearBits(device->evt_grp_spi, EVT_GRP_SPI_RX_VALID_PACKET_BIT | EVT_GRP_SPI_RX_INVALID_PACKET_BIT);
    }

    ESP_LOGD(TAG, "wait_for_data (CS_Pin=%d): Returning %s", device->imu_config.io_cs, success ? "true" : "false");
    return success;
}
...
uint8_t BNO08x_get_reset_reason(BNO08x *device) {
    uint32_t reset_reason_val = 0;
    ESP_LOGI(TAG, "get_reset_reason: Called for CS_Pin=%d", device->imu_config.io_cs);

    BNO08x_queue_request_product_id_command(device);
    ESP_LOGI(TAG, "get_reset_reason: Product ID command queued.");

    if (!BNO08x_wait_for_tx_done(device)) {
        ESP_LOGE(TAG, "get_reset_reason: Failed to send product ID report request.");
        ESP_LOGI(TAG, "get_reset_reason: Returning %" PRIu32 " for CS_Pin=%d", reset_reason_val, device->imu_config.io_cs);
        return (uint8_t)reset_reason_val;
    }

    ESP_LOGI(TAG, "get_reset_reason: Product ID command sent. Waiting for Product ID Report...");

    // Try to receive the specific report for a certain duration
    TickType_t start_time = xTaskGetTickCount();
    TickType_t timeout_ticks = pdMS_TO_TICKS(HOST_INT_TIMEOUT_MS * 3); // Allow more time, e.g., 3x the usual packet timeout

    while (xTaskGetTickCount() - start_time < timeout_ticks) {
        // Check if a reset reason has already been populated in the queue by the data_proc_task
        if (xQueueReceive(device->queue_reset_reason, &reset_reason_val, 0) == pdPASS) { // Non-blocking check
            ESP_LOGI(TAG, "get_reset_reason: xQueueReceive SUCCESS from queue_reset_reason. Reset reason = %" PRIu32, reset_reason_val);
            ESP_LOGI(TAG, "get_reset_reason: Returning %" PRIu32 " for CS_Pin=%d", reset_reason_val, device->imu_config.io_cs);
            return (uint8_t)reset_reason_val;
        }

        // If not in queue yet, wait for ANY data to arrive and be processed.
        // BNO08x_wait_for_data() consumes one packet event.
        // We are hoping the data_proc_task processes the SHTP_REPORT_PRODUCT_ID_RESPONSE
        // and puts the reason into queue_reset_reason.
        ESP_LOGD(TAG, "get_reset_reason: No reason in queue yet for CS_Pin=%d. Calling BNO08x_wait_for_data().", device->imu_config.io_cs);
        if (BNO08x_wait_for_data(device)) { // This will wait for a HINT and for data_proc_task to parse a packet
            ESP_LOGI(TAG, "get_reset_reason: BNO08x_wait_for_data() processed a packet for CS_Pin=%d. Loop will check queue again.", device->imu_config.io_cs);
            // Loop again to check queue_reset_reason with a non-blocking read.
            // Add a small delay to allow data_proc_task to run if it was just unblocked
            vTaskDelay(pdMS_TO_TICKS(10));
        } else {
            ESP_LOGW(TAG, "get_reset_reason: BNO08x_wait_for_data() timed out or returned invalid packet for CS_Pin=%d. Loop continues.", device->imu_config.io_cs);
            // If BNO08x_wait_for_data times out, it means no HINT was received for a while.
            // This could be problematic. The loop will continue until the outer timeout.
            vTaskDelay(pdMS_TO_TICKS(50)); // Give some time before retrying BNO08x_wait_for_data
        }
    }

    ESP_LOGE(TAG, "get_reset_reason: Global TIMEOUT waiting for Product ID Report for CS_Pin=%d. Reset reason remains %" PRIu32, device->imu_config.io_cs, reset_reason_val);
    ESP_LOGI(TAG, "get_reset_reason: Returning %" PRIu32 " for CS_Pin=%d", reset_reason_val, device->imu_config.io_cs);
    return (uint8_t)reset_reason_val;
}
...
bool BNO08x_receive_packet(BNO08x *device) {
    bno08x_rx_packet_t packet_to_queue; // This will be filled and sent to the queue
    esp_err_t ret;

    // Allocate DMA capable buffers for header RX and dummy TX
    uint8_t* dma_rx_header_buf = heap_caps_malloc(4, MALLOC_CAP_DMA);
    uint8_t* dma_tx_dummy_header_buf = heap_caps_malloc(4, MALLOC_CAP_DMA);

    if (!dma_rx_header_buf || !dma_tx_dummy_header_buf) {
        ESP_LOGE(TAG, "Failed to allocate DMA buffers for header");
        if (dma_rx_header_buf) heap_caps_free(dma_rx_header_buf);
        if (dma_tx_dummy_header_buf) heap_caps_free(dma_tx_dummy_header_buf);
        // Cannot set RX_DONE_BIT if we didn't even try SPI
        // Or, set error bits and RX_DONE if that's the protocol
        return false; // Indicate failure to process HINT
    }
    memset(dma_tx_dummy_header_buf, 0, 4); // Dummy data to send

    spi_transaction_t header_trans = {
        .length = 4 * 8,
        .rxlength = 4 * 8,
        .tx_buffer = dma_tx_dummy_header_buf,
        .rx_buffer = dma_rx_header_buf
    };

    gpio_set_level(device->imu_config.io_cs, 0);
    ret = spi_device_polling_transmit(device->spi_hdl, &header_trans);
    // CS de-assertion will happen after body is read, or if error occurs early

    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "SPI transmit for header failed: %s", esp_err_to_name(ret));
        gpio_set_level(device->imu_config.io_cs, 1);
        heap_caps_free(dma_rx_header_buf);
        heap_caps_free(dma_tx_dummy_header_buf);
        // Don't set RX_DONE_BIT or queue anything, let timeout handle it or return specific error.
        // For now, following original logic of queuing partial/error packet:
        packet_to_queue.length = 0; // Indicate error explicitly
        xQueueSend(device->queue_rx_data, &packet_to_queue, 0);
        xEventGroupSetBits(device->evt_grp_spi, EVT_GRP_SPI_RX_DONE_BIT);
        return true; // HINT processed, error packet queued
    }
    
    memcpy(packet_to_queue.header, dma_rx_header_buf, 4);
    heap_caps_free(dma_rx_header_buf); // Done with DMA RX header buffer
    heap_caps_free(dma_tx_dummy_header_buf); // Done with DMA TX dummy header buffer

    uint16_t shtp_total_length = (((uint16_t)packet_to_queue.header[1]) << 8) | ((uint16_t)packet_to_queue.header[0]);
    shtp_total_length &= ~(1 << 15);

#ifdef CONFIG_ESP32_BNO08x_DEBUG_STATEMENTS
    ESP_LOGI(TAG, "SHTP Header Raw: 0x%02X 0x%02X 0x%02X 0x%02X -> Total Length: %u",
             packet_to_queue.header[0], packet_to_queue.header[1], packet_to_queue.header[2], packet_to_queue.header[3], shtp_total_length);
#endif
    
    packet_to_queue.length = shtp_total_length; // Store full SHTP length

    if (shtp_total_length < 4) {
        ESP_LOGE(TAG, "Invalid SHTP packet length from header (too small): %u", shtp_total_length);
        gpio_set_level(device->imu_config.io_cs, 1);
        packet_to_queue.length = 4; // Actual SHTP length was less, but header was read
        xQueueSend(device->queue_rx_data, &packet_to_queue, 0);
        xEventGroupSetBits(device->evt_grp_spi, EVT_GRP_SPI_RX_DONE_BIT);
        return true;
    }

    uint16_t body_length_to_read = shtp_total_length - 4;

    if (body_length_to_read > RX_DATA_LENGTH) {
        ESP_LOGE(TAG, "SHTP packet body too large for buffer: declared %u bytes, buffer capacity %d bytes", body_length_to_read, RX_DATA_LENGTH);
        gpio_set_level(device->imu_config.io_cs, 1);
        packet_to_queue.length = 4; // Only header considered "validly" processed by this func
        xQueueSend(device->queue_rx_data, &packet_to_queue, 0);
        xEventGroupSetBits(device->evt_grp_spi, EVT_GRP_SPI_RX_DONE_BIT);
        return true;
    }
    ...
    if (body_length_to_read > 0) {
        uint8_t* dma_rx_body_buf = heap_caps_malloc(body_length_to_read, MALLOC_CAP_DMA);
        uint8_t* dma_tx_dummy_body_buf = heap_caps_malloc(body_length_to_read, MALLOC_CAP_DMA);

        if (!dma_rx_body_buf || !dma_tx_dummy_body_buf) {
            ESP_LOGE(TAG, "Failed to allocate DMA buffers for body");
            gpio_set_level(device->imu_config.io_cs, 1);
            if (dma_rx_body_buf) heap_caps_free(dma_rx_body_buf);
            if (dma_tx_dummy_body_buf) heap_caps_free(dma_tx_dummy_body_buf);
            // Send what we have (header)
            packet_to_queue.length = 4;
            xQueueSend(device->queue_rx_data, &packet_to_queue, 0);
            xEventGroupSetBits(device->evt_grp_spi, EVT_GRP_SPI_RX_DONE_BIT);
            return true;
        }
        memset(dma_tx_dummy_body_buf, 0, body_length_to_read);

        spi_transaction_t body_trans = {
            .length = body_length_to_read * 8,
            .rxlength = body_length_to_read * 8,
            .tx_buffer = dma_tx_dummy_body_buf,
            .rx_buffer = dma_rx_body_buf
        };
        
        ret = spi_device_polling_transmit(device->spi_hdl, &body_trans);
        if (ret != ESP_OK) {
             ESP_LOGE(TAG, "SPI transmit for body failed: %s", esp_err_to_name(ret));
             gpio_set_level(device->imu_config.io_cs, 1);
             heap_caps_free(dma_rx_body_buf);
             heap_caps_free(dma_tx_dummy_body_buf);
             packet_to_queue.length = 4; // Only header was good
             xQueueSend(device->queue_rx_data, &packet_to_queue, 0);
             xEventGroupSetBits(device->evt_grp_spi, EVT_GRP_SPI_RX_DONE_BIT);
             return true;
        }
        memcpy(packet_to_queue.body, dma_rx_body_buf, body_length_to_read);
        heap_caps_free(dma_rx_body_buf);
        heap_caps_free(dma_tx_dummy_body_buf);
    }
 ...
 * @return void, nothing to return
 */
void BNO08x_send_packet(BNO08x *device, bno08x_tx_packet_t *packet_from_queue) {
    uint8_t* dma_tx_body_buf = heap_caps_malloc(packet_from_queue->length, MALLOC_CAP_DMA);
    if (!dma_tx_body_buf) {
        ESP_LOGE(TAG, "Failed to allocate DMA buffer for sending packet");
        // How to signal error? Maybe set TX_DONE anyway to unblock caller, or have a separate error bit.
        // For now, assume caller times out if TX_DONE is not set.
        return;
    }
    memcpy(dma_tx_body_buf, packet_from_queue->body, packet_from_queue->length);

    // Using device->spi_transaction directly might be risky if another part of the code modifies it.
    // It's safer to define a local spi_transaction_t for this operation.
    spi_transaction_t trans = {
        .length = packet_from_queue->length * 8,
        .rxlength = 0,
        .tx_buffer = dma_tx_body_buf,
        .rx_buffer = NULL,
        .flags = 0
    };

    gpio_set_level(device->imu_config.io_cs, 0);
    esp_err_t ret = spi_device_polling_transmit(device->spi_hdl, &trans);
    gpio_set_level(device->imu_config.io_cs, 1);

    heap_caps_free(dma_tx_body_buf);

    if (ret == ESP_OK) {
        xEventGroupSetBits(device->evt_grp_spi, EVT_GRP_SPI_TX_DONE_BIT);
    } else {
        ESP_LOGE(TAG, "SPI send packet failed: %s", esp_err_to_name(ret));
        // TX_DONE_BIT is not set, BNO08x_wait_for_tx_done will timeout
    }
}
...
{
    ESP_LOGW(TAG, "ParsePacket START: Chan=0x%02X, Body[0]=0x%02X, Len=%u, CS_Pin=%d",
        packet->header[2], packet->body[0], packet->length, device->imu_config.io_cs);
...
uint16_t BNO08x_parse_product_id_report(BNO08x *device, bno08x_rx_packet_t *packet)
{
    ...
    ESP_LOGI(TAG, "ParseProductIDReport: CS_Pin=%d, Parsed ResetReason=0x%02" PRIx32, device->imu_config.io_cs, reset_reason);
    ESP_LOGI(TAG,
             "Product ID Info for CS_Pin=%d:                           \n\r"
             "                ---------------------------\n\r"
    ...
    if (xQueueSend(device->queue_reset_reason, &reset_reason, 0) == pdPASS) {
        ESP_LOGI(TAG, "ParseProductIDReport: Sent %" PRIu32 " to queue_reset_reason for CS_Pin=%d", reset_reason, device->imu_config.io_cs);
    } else {
        ESP_LOGE(TAG, "ParseProductIDReport: FAILED to send to queue_reset_reason for CS_Pin=%d", device->imu_config.io_cs);
    }
    return 1;
}
 ...
 * @param time_between_reports Desired time in microseconds between each report. The BNO08x will send reports according to this interval.
...
/**
 * @brief Task responsible for SPI transactions. Triggered by HINT interrupt.
 *
 * Waits for notification from HINT ISR, then processes SPI communication.
 * It prioritizes sending packets if any are in queue_tx_data, otherwise performs a receive operation.
 * Re-enables HINT interrupt after processing.
 */
...
#ifdef CONFIG_ESP32_BNO08x_DEBUG_STATEMENTS
    static uint64_t prev_hint_time = 0;
    static uint64_t current_hint_time = 0;
#endif

    while (1)
    {
        ulTaskNotifyTake(pdTRUE, portMAX_DELAY); // Wait for HINT ISR notification

#ifdef CONFIG_ESP32_BNO08x_DEBUG_STATEMENTS
        current_hint_time = esp_timer_get_time();
        if (prev_hint_time > 0) { // Avoid large number on first run
            ESP_LOGI(TAG, "spi_task notified by HINT, time since last HINT: %llu us", (current_hint_time - prev_hint_time));
        } else {
            ESP_LOGI(TAG, "spi_task notified by HINT (first notification)");
        }
        prev_hint_time = current_hint_time;
#endif

        // Check if there's a packet to send (e.g., a command)
        if (xQueueReceive(device->queue_tx_data, &tx_packet, 0) == pdPASS)
        {
            BNO08x_send_packet(device, &tx_packet);
            // BNO08x_send_packet sets EVT_GRP_SPI_TX_DONE_BIT
        }
        else
        {
            // If nothing to send, the HINT means the BNO08x has data for us
            BNO08x_receive_packet(device);
            // BNO08x_receive_packet sets EVT_GRP_SPI_RX_DONE_BIT
            // and queues data for data_proc_task via queue_rx_data
        }

        // After handling the HINT-related SPI transaction, re-enable the HINT interrupt.
        // It was disabled by the ISR to prevent re-entrancy.
        gpio_intr_enable(device->imu_config.io_int);
    }
}

/**
 * @brief Task responsible for processing received SHTP packets.
 *
 * Waits for packets in queue_rx_data (populated by BNO08x_receive_packet),
 * parses them, sets event group bits accordingly, and triggers callbacks.
 */
void BNO08x_data_proc_task(void *arg)
{
    ...
    while (1)
    {
        ...
        if (xQueueReceive(device->queue_rx_data, &packet, portMAX_DELAY) == pdPASS)
        {
            ESP_LOGI(TAG, "data_proc_task: CS_Pin=%d Received packet from queue. SHTP Packet Length (from header field)=%u",
                     device->imu_config.io_cs, packet.length);

            // Ensure packet SHTP length field itself is plausible before parsing.
            // BNO08x_receive_packet should do some validation on this to prevent huge lengths.
            // The packet.length field here refers to the SHTP total length.
            // The actual data in packet.body is limited by RX_DATA_LENGTH (for body part).
            if (packet.length >= 4 && packet.length <= (sizeof(packet.header) + RX_DATA_LENGTH) ) { // Check against actual buffer limits
                // The BNO08x_parse_packet function returns the report_ID if successful, or 0 if it's an unhandled packet
                // or an error during parsing (though specific parse errors are usually logged within BNO08x_parse_packet itself).
                uint16_t report_id_or_parse_status = BNO08x_parse_packet(device, &packet);

                if (report_id_or_parse_status != 0) // A non-zero value typically means successful parse of a known/handled report/response
                {
                    ESP_LOGI(TAG, "data_proc_task: CS_Pin=%d, BNO08x_parse_packet returned 0x%02X (VALID). Setting EVT_GRP_SPI_RX_VALID_PACKET_BIT.",
                             device->imu_config.io_cs, report_id_or_parse_status);
                    xEventGroupSetBits(device->evt_grp_spi, EVT_GRP_SPI_RX_VALID_PACKET_BIT);

                    // Call registered callbacks if any
                    // This part is generally for application-level data handling, not critical for initial handshaking.
                    if (device->cb_list != NULL && device->cb_list->length > 0)
                    {
                        for (uint16_t i = 0; i < device->cb_list->length; i++)
                        {
                            if (device->cb_list->callbacks[i] != NULL)
                            {
                                // Consider if callbacks should only be for specific report_ids later on.
                                // For now, any "valid" packet triggers callbacks.
                                device->cb_list->callbacks[i]((void *)device);
                            }
                        }
                    }
                }
                else // BNO08x_parse_packet returned 0
                {
                    // This can happen for:
                    // 1. Packets that are not explicitly handled by BNO08x_parse_packet (e.g., some unsolicited SHTP internal messages).
                    // 2. Actual parsing errors within BNO08x_parse_packet for recognized channels/reports.
                    // 3. The "Advertisement message" (often all zeros) or other low-level SHTP administrative packets.
                    ESP_LOGW(TAG, "data_proc_task: CS_Pin=%d, BNO08x_parse_packet returned 0 (e.g. unknown packet, parse error, or non-report SHTP msg). Packet SHTP Len=%u, Chan=0x%02X, Body[0]=0x%02X. Setting EVT_GRP_SPI_RX_INVALID_PACKET_BIT.",
                             device->imu_config.io_cs, packet.length, packet.header[2], packet.body[0]);
                    xEventGroupSetBits(device->evt_grp_spi, EVT_GRP_SPI_RX_INVALID_PACKET_BIT);
                }
            } else { // packet.length (SHTP total length) is out of reasonable bounds
                 ESP_LOGE(TAG, "data_proc_task: CS_Pin=%d, Received packet from queue with SHTP length out of bounds: %u. Min=4, Max supported header+body=%zu. Setting EVT_GRP_SPI_RX_INVALID_PACKET_BIT.",
                          device->imu_config.io_cs, packet.length, (sizeof(packet.header) + RX_DATA_LENGTH));
                 xEventGroupSetBits(device->evt_grp_spi, EVT_GRP_SPI_RX_INVALID_PACKET_BIT);
            }
            // EVT_GRP_SPI_RX_DONE_BIT is set by BNO08x_receive_packet after it successfully queues the data.
            // So, by the time data_proc_task runs, RX_DONE should already be set for this HINT cycle.
        }
    }
}
